...
if(칸들[몇줄][몇칸].textContent!== ""){//칸이 이미 채워져 있는가?
    ...
    //대각선 검사
    if(몇줄 === 몇칸||Math.abs(몇줄-몇칸)===2){//대각선 검사 필요한 경우 
        if(칸들[0][0].textContent===턴 &&칸들[1][1].textContent===턴&&칸들[2][2].textContent){
            다참=true;
        }
    }
    if(Math.abs(몇줄-몇칸)===2){//대각선 검사 필요한 경우 
        if(칸들[2][0].textContent===턴 &&칸들[1][1].textContent===턴&&칸들[0][2].textContent){
            다참=true;
        }
    }
    if(다참){
